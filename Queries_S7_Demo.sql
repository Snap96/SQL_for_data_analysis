...
SELECT
	event_desc,
    REGEXP_SUBSTR(event_desc, '[A-Z],[a-z]+(-[A-Za-z]+)+') AS hypen_phase
FROM my_events;
...
-- Return description that start with 'A'
SELECT *
FROM my_events
WHERE event_desc LIKE 'A %';
...
('Bob', NULL, 'bob.altexample.com'),
('Charlie', NULL, NULL),
('David', 'david@example.com', 'davidalt@example.com');
...
SELECT
	name, email, alt_email,
    IFNULL(email, alt_email) as contact_email
FROM contacts;

-- Return an alternative field after multiple checksum table
SELECT
	name, email, alt_email,
